...
func Card(suit int, rank int) int {
	return suit | rank
}
...
func CardToStr(card int) string {
	var str string
	suit := CardSuit(card)
	rank := CardRank(card)
	...
	case rank <= CardRank9 && rank >= CardRank3:
		str += string(int('3') + rank - CardRank3)
	}
...
func (cs CardSlice) Less(i, j int) bool {
	...
	// rotation
	ra = (cs[i]&0x0F0)>>4 | (cs[i]&0x0F)<<4
	rb = (cs[j]&0x0F0)>>4 | (cs[j]&0x0F)<<4
...
func (cs CardSlice) Contains(c int) bool {
	for i := 0; i < len(cs); i++ {
		if c == cs[i] {
			return true
		}
...
func (cs CardSlice) Push(card int) CardSlice {
	return append(cs, card)
}

func (cs CardSlice) Pop() (int, CardSlice) {
	if len(cs) == 0 {
		return 0, cs
	}
...
func (cs CardSlice) Shift(card int) (int, CardSlice) {
	if len(cs) == 0 {
		return 0, cs
	}
...
func (cs CardSlice) Unshift(card int) CardSlice {
...
func (cs CardSlice) Insert(i int, card int) CardSlice {
	if i < 0 || i >= len(cs) {
		return cs
	}
...
func (cs CardSlice) Remove(i int) (int, CardSlice) {
	if i < 0 || i >= len(cs) {
		return 0, cs
	}
...
func (cs CardSlice) RemoveCard(card int) (bool, CardSlice) {
	for i := 0; i < len(cs); i++ {
		if cs[i] == card {
			_, s := cs.Remove(i)
			return true, s
